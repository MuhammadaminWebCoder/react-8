...
function CardContent(navigate,id) {
  return(
    <div className='flex items-center gap-5'>
      <Button className='w-full !bg-green-600'><EditFilled className=' !text-white text-[20px]'/></Button>
...
const Stack = () => {
  ...
  return (
    <div className='p-5'>
      <Caption title={'Yonalishlar'} count={stackList.length} icon={<CodeSandboxOutlined/>} />
      <ul className='flex mt-8 flex-wrap gap-5'>
        {stackList.map((item,ind) => (
          ...
          key={ind}
            hoverable
            cover={<img onClick={() => navigate(`${item.id}`)} alt='example' className='h-[140px] object-cover' src={item.image} />}
          >
